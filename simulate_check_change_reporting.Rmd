...
if(!is.na(input$t_pi_change)){
  Pi_t <- data.frame(t = c(Rt$t,tail(Rt$t,1)+1),
                     pi = c(rep(input$pi,input$t_pi_change*input$step+1),
                            rep(input$pi2, nrow(Rt)+1-(input$t_pi_change*input$step+1) )) )
}else{
  Pi_t <- input$pi
}
...
rep <- 1e4
res_MCMC_EpiEstim_1 <- fct_MCMC_EpiEstim(I0_t_import = sim$I0$timespan , I = sim$res$I_obs, 
                                              t_window = t_window,
                                             mean_prior = mean_prior, std_prior = std_prior,
