...
res_MCMC_EpiEstim_Poiss3 <- fct_MCMC_EpiEstim(I0 = sim$I0 , I = sim$res$I_obs, #I_true , 
                                              t_window = t_window,
                                             mean_prior = mean_prior, std_prior = std_prior,
                                             res_EpiEstim = res_EpiEstim, overdispersion = FALSE, 
                                             rep = rep, thin = 10, param_agg = FALSE, p_reps = input$pi)
...
comp_plot <- function(f1,f2){
  ...
  if(f2 %in% c("Quantile.0.025(R)","Quantile.0.975(R)")){
    if(f2 %in%"Quantile.0.025(R)"){
      ...
      Rts_mcmc_NB1 <- apply(res_MCMC_EpiEstim_NB1$theta_R,2,quantile,0.025,na.rm = TRUE)
      Rts_mcmc_NB2 <- apply(res_MCMC_EpiEstim_NB2$theta_R,2,quantile,0.025,na.rm = TRUE)
    }else{
      # Rts_mcmc_pois <- apply(res_MCMC_EpiEstim_Poiss1$theta_R,2,quantile,0.975,na.rm = TRUE)
      Rts_mcmc_NB1 <- apply(res_MCMC_EpiEstim_NB1$theta_R,2,quantile,0.975,na.rm = TRUE)
      Rts_mcmc_NB2 <- apply(res_MCMC_EpiEstim_NB2$theta_R,2,quantile,0.975,na.rm = TRUE)
    }
  }else{
    # Rts_mcmc_pois <- apply(res_MCMC_EpiEstim_Poiss1$theta_R,2,f1)
    Rts_mcmc_NB1 <- apply(res_MCMC_EpiEstim_NB1$theta_R,2,f1)
    Rts_mcmc_NB2 <- apply(res_MCMC_EpiEstim_NB2$theta_R,2,f1)
  }
  ...
  plot(log10(Rts_epiestim), log10(Rts_mcmc_NB1), bty = 'n', main = f2, col = 'blue3',pch=3)
